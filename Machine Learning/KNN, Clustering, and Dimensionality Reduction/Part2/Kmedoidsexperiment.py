...
def run(dataset, k, repeats=10):
    loss=[]
    silhouette=[]
    
    for i in range(repeats):
        kmedoids=KMedoids(n_clusters=k, metric="cosine").fit(dataset)
        loss.append(kmedoids.inertia_)
        labels=kmedoids.labels_
        silhouette.append(silhouette_score(dataset, labels, metric="cosine"))
