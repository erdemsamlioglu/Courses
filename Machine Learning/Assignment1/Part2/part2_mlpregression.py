...
def mse(train_predictions, train_label):
    return torch.mean((train_predictions-train_label)**2)
...
def forward_pass(w1, b1, w2, b2, input_data):
    ...
    output_layer_output=torch.matmul(hidden,w2)+b2
...
mean, std=0, 1
w1 = torch.zeros((2,512), requires_grad=True)
...
b1 = torch.zeros((1,512), requires_grad=True)
...
w2 = torch.zeros((512,1), requires_grad=True)
...
for iteration in range(1, ITERATION+1):
    ...
    train_mse_loss = mse(train_predictions, train_label)
    ...
    with torch.no_grad():
        ...
        validation_mse_loss = mse(validation_predictions, validation_label)
...
with torch.no_grad():
    ...
    test_loss = mse(test_predictions, test_label)
