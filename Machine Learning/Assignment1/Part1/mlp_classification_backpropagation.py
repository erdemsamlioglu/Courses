...
class MLPClassifier:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        softmaxinput=np.dot(hidden_layer_output, self.GAMMA)+self.GAMMA_bias
        output_layer_output= softmax(softmaxinput)
    ...
    def train(self, data_instances: np.ndarray, labels: np.ndarray):
        ...
        for iteration_count in range(1, self.epoch_number+1):
            ...
            for data_index in range(len(data_instances)):
                ...
                output= output_layer_output - label
                GAMMA_update=np.dot(hidden_layer_output.T, output)
                GAMMA_bias_update=np.sum(output, axis=0)

                hidden=np.dot(output, self.GAMMA.T) * hidden_layer_output * (1-hidden_layer_output)
                W_update=np.dot(x.T, hidden)
                W_bias_update=np.sum(hidden, axis=0)
